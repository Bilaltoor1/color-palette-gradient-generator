...
export default async function ShadeDetailPage({ params }) {
  ...
  if (!collection) {
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <h1 className="text-2xl sm:text-3xl font-semibold mb-4">Collection not found</h1>
        <p className="text-foreground/70">We couldn't find that collection. It may have been deleted or the slug is incorrect.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      <h1 className="text-2xl sm:text-3xl font-semibold mb-4">{collection.title}</h1>
      {collection.description && (
        <p className="text-foreground/70 mb-6">{collection.description}</p>
      )}
      {colors.length === 0 ? (
        <div className="p-6 rounded border bg-white/60 dark:bg-white/5">No colors in this collection yet.</div>
      ) : (
